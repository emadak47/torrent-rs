...
use crate::exchanges::coinbase::types::Event;
use crate::exchanges::coinbase::types::L2update;
use crate::exchanges::coinbase::types::Snapshot;
...
impl BybitFeedManager {
    ...
    fn reset(&mut self, snapshot: Snapshot) -> Result<(), Error> {
    ...
    /// Takes an update event and updates the [`OrderbookL2`] corresponding to the symbol key
    fn update(&mut self, update: L2update) -> Result<(), Error> {
        ...
        for level in update.changes {
            ...
            let qty = level.size;
            if !price.is_empty() && !qty.is_empty() {
                let price_f64 = price.parse::<f64>().context(format!(
                    "Failed to parse an ask price: {:?}",
                    price
                ))?;
                let price_u64 = (price_f64 * multiplier) as u64;
                let qty_f64 = qty
                    .parse::<f64>()
                    .context(format!("Failed to parse an ask qty: {:?}", qty))?;
                let qty_u64 = (qty_f64 * multiplier) as u64;
                if qty_u64 == 0 {
                    if level.side == OrderSide::Buy {
                        book.delete(Side::SELL, price_u64);
                    } else {
                        book.delete(Side::BUY, price_u64);
                    }
                } else {
                    if level.side == OrderSide::Sell {
                        book.add(Side::SELL, price_u64, qty_u64);
                    } else {
                        book.add(Side::BUY, price_u64, qty_u64);
                    }
                }
                asks_flat.push(Level::new(price_u64, qty_u64));
            }
        }
        ...
        loop {
            match self.rx.recv().await {
                Some(event) => match event {
                    Event::Snapshot(snapshot_data) => {
                        self.reset(snapshot_data);
                    }
                    Event::L2update(l2update_data) => {
                        self.update(l2update_data);
                    }
                },
                None => {
                    continue;
